...
export async function POST(req: NextRequest, { params }: Props) {
  ...
  const comment = await Comment.findOneAndUpdate(
    { "reply._id": cid },
    {
      $addToSet: { "reply.$.Hearts": { email: email } }, // 사용자의 이메일을 Hearts 배열에 추가합니다.
      $inc: { "reply.$._count.Hearts": 1 }, // Hearts 수를 1 증가시킵니다.
    },
...
export async function DELETE(req: NextRequest, { params }: Props) {
  ...
  const comment = await Comment.findOneAndUpdate(
    { "reply._id": cid },
    {
      $pull: { "reply.$.Hearts": { email: email } },
      $inc: { "reply.$._count.Hearts": -1 },
    },
